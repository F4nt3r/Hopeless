...
namespace Hopeless
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            Cleric cleric = new Cleric("Cleric",0,2,5,10,45,45,40,40,30,30,1,2,15, CharacterType.Cleric);
            Joker joker = new Joker("Joker",0,2,7,8,30,30,30,30,50,50,1,2,20, CharacterType.Joker);
            ...
            Monster maggot = new Monster("Robal", 20,20,20,10,10,20,2,4,33,DifficultyType.Easy);        
            Monster webber = new Monster("Pajak",20,15,15,10,10,30,2,4,30,DifficultyType.Easy);  
            Monster webberSpitter = new Monster("Pajak_Trujacy",25,25,25,15,10,35,5,7,35, DifficultyType.Easy);         
            Monster brigandTrainee = new Monster("Poczatkujacy_Bandyta", 25, 20, 20, 15, 10, 10, 3, 5, 15, DifficultyType.Easy);
            ...
            Monster pijawka = new Monster("Pijawka", 20, 25, 25, 10, 15, 25, 3, 6, 30, DifficultyType.Easy);
            Monster voider = new Monster("Otchlaniec", 15, 15, 15, 10, 10, 10, 2, 3, 25, DifficultyType.Easy);
            Monster tentacles = new Monster("Macki_Otchlani", 15, 10, 10, 10, 10, 25, 4, 5, 30, DifficultyType.Easy);
            Monster wildDog = new Monster("Dziki_Pies", 10, 20, 20, 15, 15, 10, 5, 6, 10, DifficultyType.Easy);


            Monster boneShielder = new Monster("Szkielet_Tarczownik", 20, 30, 30, 10, 10, 20, 2, 4, 33, DifficultyType.Medium);
            Monster gargoyle = new Monster("Gargulec", 45, 40, 40, 15, 15, 10, 7, 10, 25, DifficultyType.Medium);
            Monster boneSoldier = new Monster("Szkielet_Wojownik", 45, 30, 30, 10, 15, 15, 5, 11, 10, DifficultyType.Medium);
            Monster boneArbalest = new Monster("Szkielet_Kusznik", 40, 25, 25, 10, 20, 15, 6, 9, 10, DifficultyType.Medium);
            ...
            Monster orkwarrior = new Monster("Ork_Wojownik", 40, 35, 35, 15, 20, 10, 8, 10, 20, DifficultyType.Medium);
            Monster pokutnik = new Monster("Pokutnik", 35, 40, 40, 15, 15, 15, 7, 9, 15, DifficultyType.Medium);
            Monster arachas = new Monster("Kultysta", 40, 55, 55, 35, 10, 10, 6, 7, 10, DifficultyType.Medium);

            Monster giant = new Monster("Gigant", 75, 55, 55, 30, 20, 10, 12, 16, 5, DifficultyType.Hard);

            Monster armata = new Monster("Armata", 75, 55, 55, 40, 15, 10, 10, 13, 5, DifficultyType.Hard);

            Monster ghost = new Monster("Widmo", 80, 50, 50, 20, 15, 20, 11, 15, 30, DifficultyType.Hard);

            Monster bigCrab = new Monster("Wielki_Krab", 75, 55, 55, 15, 15, 15, 9, 14, 10, DifficultyType.Hard);
            Monster Leszy = new Monster("Leszy", 70, 55, 55, 25, 15, 15, 10, 16, 10, DifficultyType.Hard);
            Monster vampire = new Monster("Wampir_Wyzszy", 50, 45, 45, 30, 10, 15, 11, 15, 20, DifficultyType.Hard);

            Monster sleeper = new Monster("Sniacy", 100,300,300,50,15,50,20,28,15, DifficultyType.Boss);
            Monster sleeperGuard = new Monster("Straznik_Sniacego",100,150,150,40,15,40,12,17,10,DifficultyType.Boss);
            Monster gaunter = new Monster("Gaunter_oDim", 100, 2, 2, 50, 20, 50, 17, 22, 15, DifficultyType.Event);
            Monster mirage = new Monster("Odbicie_Lustrzane",100,2,2,40,20,40,14,19,15, DifficultyType.Event);
